package org.mj.flink.example.batch
...
object WordCount {
  ...
  def main(args: Array[String]): Unit = {
    println("Running flink WordCount exmaple")
...
//    val env = ExecutionEnvironment.getExecutionEnvironment
    ...
    val word7Groups: GroupedDataSet[(String, Int)] = wordsPairs.groupBy {
      _ match {
        case (s, i) =>
    ...
    println(s"max value in each modulo class: ${
      xs.groupBy { _ % 7 }
    ...
    val xsPlus = xs.map(new RichMapFunction[Int, Int] {
      ...
      override def open(parameters: Configuration): Unit = {
        LOG.warn(s"starting f with parameters $parameters")
        LOG.warn("""this is a nice place for opening a database 
           connection, or creating a Jackson mapper, to say something""")
      }

      override def close(): Unit = {
        LOG.warn(s"closing f")
      }
    })
